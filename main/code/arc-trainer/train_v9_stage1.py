...
base_path = '/ocean/projects/cis250063p/jbentley/ARC-AGI-2/Capstone-ARC2/shared/arc/data/ARC-Data/input/stage1/input'
# input paths
#base_model = 'nvidia/Mistral-NeMo-Minitron-8B-Base'  # auto-downloaded from huggingface.co
arc_cdg_path = os.path.join(base_path, 'ARC-CDG')  # as on kaggle arc prize 2024
...
base_model = '/ocean/projects/cis250063p/jbentley/ARC-AGI-2/Capstone-ARC2/shared/arc/outputs/models/Mistral-Minitron-8B-Stage-0-merged'  # <-- local merged model dir

save_model_path = os.path.join(base_model_path, "Mistral-Minitron-8B-Stage-1")
    ...
    for action in ['train', 'merge']:
        ...
        if action == 'train':
            ...
            if rank == 0:
                ...
                arc_eval_set = arc_eval_set.load_solutions(os.path.join(arc_data_path, 'arc-agi_evaluation_solutions.json'))
                concept_arc = ArcDataset.load_from_neoneye(os.path.join(neoneye_path, "dataset", "ConceptARC"))
                arc_cdg_set = ArcDataset.load_from_json(os.path.join(arc_cdg_path, 'arc_cdg_challenges.json'))
                arc_cdg_set = arc_cdg_set.load_solutions(os.path.join(arc_cdg_path, 'arc_cdg_solutions.json'))
            
                  
                mix_datasets = {
                    "arceval": arc_eval_set.move_test_to_train().repeat(1024),
                    "concept": concept_arc.move_test_to_train().repeat(128),
                    "arcCDG": arc_cdg_set.move_test_to_train()
                }
