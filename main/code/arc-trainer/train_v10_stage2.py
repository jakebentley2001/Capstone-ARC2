...
base_path = '/ocean/projects/cis250063p/jbentley/ARC-AGI-2/Capstone-ARC2/shared/arc/data/ARC-Data/input/stage2/input'
# input paths
#base_model = 'nvidia/Mistral-NeMo-Minitron-8B-Base'  # auto-downloaded from huggingface.co
arc_2_data_path = os.path.join(base_path, 'arc-prize-2025')  # as on kaggle arc prize 2024
arc_data_path = os.path.join(base_path, 'arc-prize-2024')  # as on kaggle arc prize 2024
re_arc_path = os.path.join(base_path, 'RE-ARC')  # https://github.com/michaelhodel/re-arc
...
base_model = '/ocean/projects/cis250063p/jbentley/ARC-AGI-2/Capstone-ARC2/shared/arc/outputs/models/Mistral-Minitron-8B-Stage-1-merged'  # <-- local merged model dir

save_model_path = os.path.join(base_model_path, "Mistral-Minitron-8B-Stage-2")
...
def download_model(model_name):
    logger.info(f"Local model path detected; skipping pre-download: {model_name}")
...
def merge_lora_weights(base_model_path, adapter_path, output_path):
    """Merge LoRA into a local base model."""
    logger.info(f"Merging LoRA weights from {adapter_path} into base model: {base_model_path}")
    start_time = time.time()
    ...
    added_pad = False
    if tokenizer.pad_token is None:
        logger.info("Adapter tokenizer has no pad_token. Adding <pad>…")
        tokenizer.add_special_tokens({"pad_token": "<pad>"})
        added_pad = True
    ...
    # Keep your embedding shrink logic if you need it
    keep_tok = list('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789!?.:,;*+/-=') + tokenizer.tokenize('\n')
    keep_single_char_tokens(model, tokenizer, keep=keep_tok, remove_unk=True)

    # If we added pad above, ensure size is consistent (after any shrink).
    if added_pad:
        model.resize_token_embeddings(len(tokenizer))
...
def main():
    ...
    for action in ['train', 'merge']:
        ...
        if action == 'train':
            ...
            # ========= Pattern A: rank-0 builds once; all ranks load + shard =========
            logger.info("Loading and preparing training data")

            import datasets  # ok to import here; avoid re-importing os/dist inside functions

            # rank/world helpers (no dist.barrier needed; we poll for the saved dataset)
            ...
            def tokenize_fn(ds):
                # Tokenize streamed rows; keep RAM low
                return ds.map(
                    lambda ex: tokenizer(
                        ex["text"],
                        padding=False,
                    ...
                    remove_columns=["text"],
                    num_proc=4,              # only used on rank 0 (others don't build)
            ...
            if rank == 0:
                ...
                arc2_train_set = ArcDataset.load_from_json(os.path.join(arc_2_data_path, 'arc-agi_training_challenges.json'))
                arc2_train_set = arc2_train_set.load_solutions(os.path.join(arc_2_data_path, 'arc-agi_training_solutions.json'))
              
                arc2_eval_set = ArcDataset.load_from_json(os.path.join(arc_2_data_path, 'arc-agi_evaluation_challenges.json'))
                arc2_eval_set = arc2_eval_set.load_solutions(os.path.join(arc_2_data_path, 'arc-agi_evaluation_solutions.json'))
                  
                mix_datasets = {
                    "arceval": arc_eval_set.move_test_to_train().repeat(128),
                    "concept": concept_arc.move_test_to_train().repeat(128),
                    "arc2train": arc_eval_set.move_test_to_train().repeat(64),
                    "arc2eval": arc_eval_set.move_test_to_train().repeat(512),
                ...
                # 3) Augment keys (lazy; no strings yet)
                logger.info("Augmenting training data")
                train_dataset_augment = train_dataset.augment(
                    tp=True, rt=True, perm=True, shfl_ex=True, seed=0
                )
                ...
                # atomic replace so other ranks don't read a half-written dir
                if os.path.exists(disk_ds_dir):
